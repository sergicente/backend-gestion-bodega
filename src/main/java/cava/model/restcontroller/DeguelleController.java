...
public class DeguelleController {
	...
	public ResponseEntity<?> insertarUno(@RequestBody DeguelleDto dto) {
		...
		for (MaterialCava mat : lista) {
			...
			mov.setDescripcion(
					"Desgorg " + dto.getCantidad() + " ampolles de " + cava.getNombre() + " (" + dto.getLot() + ")");
	...
	public ResponseEntity<?> modificar(@PathVariable Long id, @RequestBody DeguelleDto dto) {
		...
		partidaAnterior
				.setBotellasRima(partidaAnterior.getBotellasRima() + existente.getCantidad() + existente.getMerma());
		...
		Optional<CavaPartida> cpAnteriorOptional = cpservice.buscarPorCavaYPartida(cavaAnterior.getId(),
				partidaAnterior.getId());
		...
		if (rimaDisponible < totalNecesario) {
			return ResponseEntity.status(HttpStatus.BAD_REQUEST)
					.body("No hay suficientes botellas en rima en la nueva partida");
		}
		...
		for (MaterialCava matCava : materialesActuales) {
			...
			nuevoMov.setDescripcion("Actualització desgorg " + dto.getCantidad() + " ampolles de "
					+ nuevaCava.getNombre() + " (" + dto.getLot() + ")");
		...
		if (nuevoTotalDeguellado < cp.getVendido()) {
			throw new IllegalArgumentException(
					"No se puede cambiar el degüelle de cava: hay " + cp.getVendido() +
							" botellas vendidas de " + cava.getNombre() + " (" + partida.getId()
							+ "), pero quedarían solo " +
	...
	public ResponseEntity<?> borrar(@PathVariable Long id) {
		...
		try {
			...
			Optional<CavaPartida> relacionAnterior = cpservice.buscarPorCavaYPartida(cavaAnterior.getId(),
					partidaAnterior.getId());
	...
	// 		int cantidadRealDisponible = material.getCantidad()
	// 				+ Math.round(material.getCantidadGastada() * cantidadYaDescontada);
	...
	private void verificarReversionPosible(Deguelle deg, int nuevaCantidad) {
		...
		// Calcular el total de botellas degüelladas para esa combinación (incluyendo
		// mermas)
		List<Deguelle> deguellesRelacionados = dservice.buscarPorCavaYPartida(cava.getId(), partida.getId());
		int totalDeguellado = deguellesRelacionados.stream()
				.mapToInt(d -> d.getCantidad() + d.getMerma())
		...
		if (nuevoTotalDeguellado < cp.getVendido()) {
			throw new IllegalArgumentException("Hi ha " + cp.getVendido() + " ampolles venudes, però quedarien sols "
					+ nuevoTotalDeguellado + " desgorjades.");
		}
	}
